...
#[test]
fn test_cat_vfs_read() {
    // Boot cat.elf which reads test.txt from the host VFS via
    // the GEO_VFS_READ SBI hypercall and prints its contents.
    //
    // Prerequisites:
    //   - examples/riscv-hello/cat.elf (built with ./build.sh cat.c)
    //   - .geometry_os/fs/test.txt (test fixture)
    let elf_path = "examples/riscv-hello/cat.elf";
    let elf_data = match std::fs::read(elf_path) {
        Ok(d) => d,
        Err(_) => {
            // Try building it
            ...
            match status {
                Some(s) if s.success() => {
                    ...
                    match std::fs::read(&p) {
                        Ok(d) => {
                            // Copy to cat.elf for future runs
                            let _ = std::fs::copy(&p, elf_path);
                            d
                        }
                        Err(e) => panic!("Built cat but can't read: {} ({:?})", e, p),
                    }
                }
                _ => panic!(
                    "Cannot build or find {} -- is riscv64-linux-gnu-gcc installed?",
    ...
    assert!(
        fixture.exists(),
        "Test fixture missing: {:?} -- create it with test.txt content",
        fixture
    );

    eprintln!("cat ELF size: {} bytes", elf_data.len());
    ...
    // Collect all characters from the canvas rows
    let mut uart_str = String::new();
    for r in 0..32 {
    ...
    // The cat program should print the file contents
    assert!(
        uart_str.contains("Hello from Geometry OS VFS"),
        "Expected VFS file contents in UART output, got: {:?}",
        uart_str
    );
    assert!(
        uart_str.contains("cat: done"),
