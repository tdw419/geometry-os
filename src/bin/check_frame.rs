...
fn main() {
    ...
    while pc < asm.pixels.len() {
        ...
        if op == 0x02 {
            ...
            let next = if pc + 1 < asm.pixels.len() { asm.pixels[pc+1] } else { 0 };
            let next2 = if pc + 2 < asm.pixels.len() { asm.pixels[pc+2] } else { 0 };
        ...
        let width = match op {
            ...
            0x03|0x10|0x11|0x12|0x15|0x16|0x17|0x20..=0x29|0x2B|0x31|0x32|0x35|0x36|0x4B|0x50|0x51 => 3,
            0x13|0x18|0x19|0x1A|0x1B|0x1C|0x2A|0x30|0x33|0x42|0x47|0x48|0x49|0x4D|0x4E|0x60|0x61 => 2,
