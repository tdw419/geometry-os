// ============================================================================
// GPU-NATIVE SOCIETY SHADER (v2 - Persistence Enabled)
// ============================================================================
...
@group(0) @binding(0) var<storage, read_write> agents: array<AgentState, 8>;
...
@group(0) @binding(3) var<storage, read_write> mailboxes: array<atomic<u32>, 8>;
@group(0) @binding(4) var<uniform> config: Config;

@compute @workgroup_size(8)
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let agent_id = global_id.x;
    if (agent_id >= 8u) { return; }
    ...
    switch (opcode) {
        ...
        case 11u: {
            ...
            state.pos_x = u32(i32(state.pos_x) + dx);
            state.pos_y = u32(i32(state.pos_y) + dy);
            state.pos_x = clamp(state.pos_x, 10u, config.width - 10u);
            state.pos_y = clamp(state.pos_y, 60u, config.height - 10u);
        }
        case 12u: {
            let idx = state.pos_y * config.width + state.pos_x;
            let val = atomicLoad(&framebuffer[idx]);
            state.registers[dest] = select(0u, 1u, val != 0u);
        }
        case 13u: {
            let idx = state.pos_y * config.width + state.pos_x;
            let color = state.registers[src1];
            atomicStore(&framebuffer[idx], color);
        }
        case 14u: {
            let target_id = state.registers[src1] % 8u;
            let val = state.registers[src2];
            atomicStore(&mailboxes[target_id], val);
        }
        case 15u: {
            let val = atomicExchange(&mailboxes[agent_id], 0u);
            state.registers[dest] = val;
        }
        case 16u: {
            let v1 = state.registers[src1];
            let v2 = state.registers[src2];
            if (v1 == v2) { state.flags = 1u; }
            else if (v1 > v2) { state.flags = 2u; }
            else { state.flags = 4u; }
        }
        case 17u: { 
            state.halted = 2u; // SNAPSHOT SIGNAL
        }
        default: { }
    }
