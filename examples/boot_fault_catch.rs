fn main() {
    ...
    let (mut vm, _) = RiscvVm::boot_linux(&kernel_image, initramfs.as_deref(), 256, 16_900_000, bootargs).unwrap();

    println!("At 16.9M: PC=0x{:08X} priv={:?}", vm.cpu.pc, vm.cpu.privilege);
    ...
    for count in 16_900_000..17_500_000 {
        ...
        if satp != last_satp {
            println!("[{}] SATP changed: 0x{:08X} -> 0x{:08X}", count, last_satp, satp);
            last_satp = satp;
        }
        ...
        if count % 10000 == 0 {
            ...
            if pte != last_l1_pte {
                println!("[{}] L1[770] changed: 0x{:08X} -> 0x{:08X} (PC=0x{:08X})",
        ...
        match step_result {
            StepResult::FetchFault | StepResult::LoadFault | StepResult::StoreFault => {
                if !fault_found {
                    ...
                    // Dump L1[770] NOW
                    let pte = vm.bus.read_word(l1_addr).unwrap_or(0);
                    println!("L1[770] = 0x{:08X} V={} R={} W={} X={}", pte,
                        pte & 1, (pte>>1)&1, (pte>>2)&1, (pte>>3)&1);
                    ...
                    println!("\nsepc=0x{:08X} VPN[1]={} VPN[0]={}", sepc, vpn1_sepc, vpn0_sepc);
                    println!("L1[{}] = 0x{:08X} V={}", vpn1_sepc, l1_pte_sepc, l1_pte_sepc & 1);
                    ...
                    if (l1_pte_sepc & 1) != 0 && ((l1_pte_sepc & 0xE) != 0) {
                        ...
                        println!("L2[{}] at PA=0x{:08X} = 0x{:08X} V={} R={} W={} X={}",
                            vpn0_sepc, l2_addr, l2_pte, l2_pte & 1,
                            (l2_pte>>1)&1, (l2_pte>>2)&1, (l2_pte>>3)&1);
                    }
                    ...
                    for i in 0..5 {
                        let sr = vm.step();
                        println!("  [{}] PC=0x{:08X} priv={:?} scause=0x{:08X} stvec=0x{:08X} => {:?}",
                            count + i + 1, vm.cpu.pc, vm.cpu.privilege,
                            vm.cpu.csr.scause, vm.cpu.csr.stvec, sr);
                    }
                    break;
                }
            }
