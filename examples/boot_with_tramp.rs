...
fn main() {
    ...
    while count < max_instructions {
        ...
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == geometry_os::riscv::cpu::Privilege::Machine {
            ...
            if cause_code == 11 || cause_code == 9 {
                ...
                if stvec != 0 {
                    ...
                    if forward_count <= 10 {
                        eprintln!("[diag] Forward #{} at count={}: cause={} sepc=0x{:08X} stvec=0x{:08X}",
        ...
        if count % 100000 == 0 && count > 0 {
            eprintln!("[diag] count={} PC=0x{:08X} priv={:?} satap=0x{:08X}",
    ...
    eprintln!("[diag] Final: count={} PC=0x{:08X} priv={:?} satap=0x{:08X} forwards={}",
