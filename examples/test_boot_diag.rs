...
fn main() {
    ...
    println!("PA 0x10DA (auipc a3): 0x{:08X} (expect 0x00210197)", val_10da);
    println!("PA 0x10DE (addi a3):  0x{:08X} (expect 0xE3A68693)", val_10de);
    println!("PA 0x10F6 (JAL->NOP): 0x{:08X} (expect 0x00000013)", val_10f6);
    println!("PA 0x10FA (JAL->NOP): 0x{:08X} (expect 0x00000013)", val_10fa);
    ...
    while count < max {
        ...
        if vm.cpu.pc == pc_before {
            ...
            if instr & 0x7F == 0x73 {
                ecall_count += 1;
                if ecall_count <= 5 {
                    println!("ECALL #{} at PC=0x{:08X} instr=0x{:08X}", ecall_count, vm.cpu.pc, instr);
                }
            }
        }
        ...
        if count % report_interval == 0 {
            println!("After {} instrs: PC=0x{:08X} SP=0x{:08X} stvec=0x{:08X} ecalls={}",
                count, vm.cpu.pc, vm.cpu.x[2], vm.cpu.csr.read(0x005), ecall_count);
        }
    }
