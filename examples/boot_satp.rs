fn main() {
    ...
    for count in 0..target {
        ...
        if vm.cpu.csr.satp != satp_before {
            println!("[SATP CHANGE] count={} PC=0x{:08X} old=0x{:08X} new=0x{:08X}",
                count, vm.cpu.pc, satp_before, vm.cpu.csr.satp);
            last_satp = vm.cpu.csr.satp;
        }
        match result {
            StepResult::Ebreak => { println!("EBREAK at count={}", count); break; }
            _ => {}
        }
    }
    ...
    println!("scause=0x{:X} sepc=0x{:08X} stval=0x{:08X} stvec=0x{:08X}",
    ...
    for count in target..(target + 200) {
        ...
        if vm.cpu.csr.satp != satp_before {
            println!("[SATP] count={} PC=0x{:08X} 0x{:08X}->0x{:08X}", count, pc_before, satp_before, vm.cpu.csr.satp);
        }
        
        if pc_before == 0xC08EFF1C {
            println!("[HANDLER] count={} PC=0x{:08X} result={:?}", count, pc_before, result);
            println!("  scause=0x{:X} sepc=0x{:08X} stval=0x{:08X}", 
                vm.cpu.csr.scause, vm.cpu.csr.sepc, vm.cpu.csr.stval);
        }
        
        match result {
            StepResult::FetchFault | StepResult::LoadFault | StepResult::StoreFault => {
                println!("[FAULT {}] {:?} at PC=0x{:08X} scause=0x{:X} sepc=0x{:08X} stval=0x{:08X}",
                    count, result, pc_before, vm.cpu.csr.scause, vm.cpu.csr.sepc, vm.cpu.csr.stval);
            }
            StepResult::Ebreak => { println!("[EBREAK] count={}", count); break; }
            _ => {}
        }
    }
}
