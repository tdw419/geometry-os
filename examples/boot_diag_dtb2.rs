...
fn main() {
    ...
    for i in 0..40usize { if dtb_bytes[i] != 0 { nonzero_header += 1; } }
    let mut nonzero_struct = 0;
    for i in off_struct as usize..std::cmp::min(off_struct as usize + 200, dtb_bytes.len()) { if dtb_bytes[i] != 0 { nonzero_struct += 1; } }
    ...
    for i in strings_start..strings_end { if dtb_bytes[i] != 0 { nonzero_strings += 1; } }
    
    eprintln!("  Non-zero bytes: header={} struct={} strings={}", nonzero_header, nonzero_struct, nonzero_strings);
    
    // Show strings area
    eprintln!("\n  Strings area (0x{:04X}-0x{:04X}):", strings_start, strings_end);
    ...
    while pos < dtb_bytes.len() {
        let token = u32::from_be_bytes([
            dtb_bytes[pos], dtb_bytes[pos+1], 
            dtb_bytes[pos+2], dtb_bytes[pos+3]
        ]);
        match token {
            0x00000001 => { eprintln!("    [0x{:04X}] FDT_BEGIN_NODE", pos); pos += 4; }
            0x00000002 => { eprintln!("    [0x{:04X}] FDT_END_NODE", pos); pos += 4; }
            0x00000003 => {
                let name_off = u32::from_be_bytes([
                    dtb_bytes[pos+4], dtb_bytes[pos+5],
                    dtb_bytes[pos+6], dtb_bytes[pos+7]
                ]);
                ...
                let name_off = u32::from_be_bytes([
                    dtb_bytes[pos+8], dtb_bytes[pos+9],
                    dtb_bytes[pos+10], dtb_bytes[pos+11]
                ]);
                ...
                if name.contains("initrd") || name.contains("bootargs") {
                    ...
                    if len > 0 && len < 200 {
                        ...
                        if val.iter().all(|&b| b >= 0x20 && b < 0x7f || b == 0) {
                            ...
                            if len == 8 {
                                let addr = u64::from_be_bytes([val[0],val[1],val[2],val[3],val[4],val[5],val[6],val[7]]);
            ...
            0x00000009 => { eprintln!("    [0x{:04X}] FDT_END", pos); break; }
            _ => { pos += 4; }
        }
        if pos > dtb_bytes.len() - 4 { break; }
    }
}
