...
fn main() {
    ...
    for i in 0..e_phnum {
        ...
        if off + e_phentsize > kernel_image.len() { break; }
        
        let p_type = u32::from_le_bytes(kernel_image[off..off+4].try_into().unwrap());
        if p_type != 1 { continue; } // PT_LOAD
        
        let p_offset = u32::from_le_bytes(kernel_image[off+4..off+8].try_into().unwrap()) as usize;
        let p_filesz = u32::from_le_bytes(kernel_image[off+16..off+20].try_into().unwrap()) as usize;
        ...
        while j + 4 <= data.len() {
            ...
            if opcode == 0x07 || opcode == 0x27 || opcode == 0x43 || 
               opcode == 0x47 || opcode == 0x4B || opcode == 0x4F || opcode == 0x53 {
                f_ext_count += 1;
    ...
    println!("Total instruction-sized words scanned: {}", total_instr_words);
    ...
    if f_ext_count > 0 {
        ...
        for (vaddr, word) in &f_ext_examples {
            let opcode = word & 0x7F;
            let opnames = [(0x07,"LOAD-FP"),(0x27,"STORE-FP"),(0x43,"FMADD"),
                          (0x47,"FMSUB"),(0x4B,"FNMSUB"),(0x4F,"FNMADD"),(0x53,"OP-FP")];
            let name = opnames.iter().find(|(op,_)| *op == opcode).map(|(_,n)| *n).unwrap_or("?");
            println!("  0x{:08X}: 0x{:08X} ({})", vaddr, word, name);
        }
        
        println!("\n*** THE KERNEL CONTAINS F-EXTENSION INSTRUCTIONS! ***");
        println!("Our interpreter is RV32IMAC (no FPU). These will cause Illegal Instruction traps.");
    ...
    for i in 0..e_phnum {
        ...
        while j + 4 <= data.len() {
            ...
            let valid = matches!(opcode, 
                0x37 | 0x17 | 0x6F | 0x67 | 0x63 | 0x03 | 0x23 | 0x13 | 
                0x33 | 0x73 | 0x0F | 0x2F);
            if !valid {
