fn main() {
    ...
    if let Some((ppn, flags)) = vm.cpu.tlb.lookup(dst_vpn, asid) {
        ...
        eprintln!("TLB: dst VA 0x{:08X} -> VPN {} -> PPN 0x{:X} flags 0x{:X} -> PA 0x{:X}",
    ...
    if let Some((ppn, flags)) = vm.cpu.tlb.lookup(x29_vpn, asid) {
        ...
        eprintln!("TLB: x29 VA 0x{:08X} -> VPN {} -> PPN 0x{:X} flags 0x{:X} -> PA 0x{:X}",
    ...
    if let Some((ppn, flags)) = vm.cpu.tlb.lookup(a4_vpn, asid) {
        ...
        eprintln!("TLB: a4  VA 0x{:08X} -> VPN {} -> PPN 0x{:X} flags 0x{:X} -> PA 0x{:X}",
    ...
    eprintln!("  L1[{}] = 0x{:08X} (PA of PTE: 0x{:X})", l1_idx, l1_pte, pt_base + (l1_idx as u64) * 4);
    ...
    if l1_v && (l1_r || l1_x) {
        ...
        eprintln!("  L1 MEGAPAGE: PPN=0x{:X} offset=0x{:X} -> PA=0x{:X}", l1_ppn, offset, pa);
        ...
        eprintln!("  L2[{}] = 0x{:08X} (PA of PTE: 0x{:X})", l2_idx, l2_pte, l2_base + (l2_idx as u64) * 4);
        eprintln!("  L2 PPN=0x{:X} offset=0x{:X} -> PA=0x{:X}", l2_ppn, offset, pa);
        eprintln!("  Expected identity-mapped PA: 0x{:X}", x29 as u64);
        eprintln!("  MATCH: {}", pa == x29 as u64);
    }
    ...
    let l1_pte_dst = vm.bus.read_word(pt_base + (l1_idx_dst as u64) * 4).unwrap_or(0);
    eprintln!("\nPage table walk for dst=0x{:08X}:", dst_va);
    eprintln!("  L1[{}] = 0x{:08X}", l1_idx_dst, l1_pte_dst);
    if (l1_pte_dst & mmu::PTE_V) != 0 && ((l1_pte_dst & mmu::PTE_R) != 0 || (l1_pte_dst & mmu::PTE_X) != 0) {
