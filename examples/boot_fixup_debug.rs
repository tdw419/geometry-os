    ...
    while count < max {
        ...
        if cur_satp != last_satap {
            ...
            println!("\n[diag] SATP change #{}: 0x{:08X} -> 0x{:08X} at count={}",
                fixup_count + 1, last_satap, cur_satp, count);
            ...
            if let Ok(pte) = vm.bus.read_word(pg_dir_phys + 771 * 4) {
                ...
                println!("[diag] L1[771] BEFORE fixup: PTE=0x{:08X} PPN=0x{:06X} ({})",
                    pte, ppn, if ppn >= 0xC0000 { "NEEDS FIXUP" } else { "OK" });
            }
            ...
            if let Ok(pte) = vm.bus.read_word(pg_dir_phys + 771 * 4) {
                let ppn = (pte >> 10) & 0x3FFFFF;
                println!("[diag] L1[771] AFTER fixup: PTE=0x{:08X} PPN=0x{:06X}", pte, ppn);
            }
            ...
            if let Ok(pte) = vm.bus.read_word(pg_dir_phys + 771 * 4) {
                let ppn = (pte >> 10) & 0x3FFFFF;
                println!("[diag] L1[771] AFTER identity inject: PTE=0x{:08X} PPN=0x{:06X}", pte, ppn);
            }
        ...
        if matches!(step_result, geometry_os::riscv::cpu::StepResult::FetchFault
