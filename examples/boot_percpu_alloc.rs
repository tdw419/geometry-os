...
fn main() {
    ...
    while count < limit {
        ...
        if pc == target_ret_addr && !found {
            let a0 = vm.cpu.x[10];
            let pa = if a0 >= 0xC0000000 { a0 - 0xC0000000 } else { a0 };
            eprintln!("memblock_alloc_from returned: VA=0x{:08X} PA=0x{:08X} (at count={})", a0, pa, count);
        ...
        if pc == 0xC040B0B6 {
            ...
            eprintln!("  a0=0x{:08X} s4=0x{:08X} s5=0x{:08X} s1=0x{:08X}", a0, s4, s5, s1);
            ...
            eprintln!("  chunk.start_offset=0x{:08X} chunk.end_offset=0x{:08X}", start_offset, end_offset);
