...
fn main() {
    ...
    for i in 0..max {
        ...
        // Track SBI ECALLs
        if vm.cpu.pc as u64 == _fw_addr && vm.cpu.privilege == geometry_os::riscv::cpu::Privilege::Machine {
            ...
            if cause_code == 11 {
                ...
                let a6 = vm.cpu.x[16];
                let a0 = vm.cpu.x[10];
                println!("[{}] ECALL_M: a7={:#x} a6={:#x} a0={:#x} (from PC={:#x})", 
                    i, a7, a6, a0, prev_pc);
        ...
        if vm.cpu.pc == last_unique_pc {
            ...
            if same_pc_count > 100 {
                println!("[{}] Stuck at PC={:#x} for {} steps, prev_pc={:#x}", 
        ...
        if same_pc_count > 10000 {
            ...
            println!("  mcause={:#x} mepc={:#x} privilege={:?}", 
                vm.cpu.csr.mcause, vm.cpu.csr.mepc, vm.cpu.privilege);
            println!("  sepc={:#x} scause={:#x} stval={:#x}", 
    ...
    println!("mcause: {:#x}, mepc: {:#x}", vm.cpu.csr.mcause, vm.cpu.csr.mepc);
    println!("scause: {:#x}, sepc: {:#x}, stval: {:#x}", 
    ...
    let unique_pcs: Vec<u32> = pc_history.iter().copied()
        .fold(Vec::new(), |mut acc, pc| {
            if acc.last() != Some(&pc) { acc.push(pc); }
            acc
        });
