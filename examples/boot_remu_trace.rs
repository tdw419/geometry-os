...
fn main() {
    ...
    let mut count: u64 = 0;
    let max_instructions: u64 = 177600;
    ...
    while count < max_instructions {
        ...
        // Log key instructions in relocate_enable_mmu
        let pc = vm.cpu.pc;
        if pc >= 0x1000 && pc <= 0x1080 && count > 177240 {
            eprintln!("[remu] count={} PC=0x{:08X} ra=0x{:08X} a0=0x{:08X} a1=0x{:08X} a2=0x{:08X} satp=0x{:08X}",
                count, pc, vm.cpu.x[1], vm.cpu.x[10], vm.cpu.x[11], vm.cpu.x[12], vm.cpu.csr.satp);
        }
