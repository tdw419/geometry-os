    ...
    while count < max_count {
        ...
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == Privilege::Machine {
            ...
            if cause_code == csr::CAUSE_ECALL_M || cause_code == csr::CAUSE_ECALL_S {
                ...
                eprintln!("[{}] SBI call #{}: ext=0x{:02X} fn={}", count, sbi_count, vm.cpu.x[17], vm.cpu.x[16]);
            } else {
                ...
                if mpp != 3 && cause_code != 0 {
                    ...
                    if stvec != 0 {
                        ...
                        vm.cpu.tlb.flush_all();
                        forward_count += 1;
                        eprintln!("[{}] FWD: cause={} mepc=0x{:08X} stvec=0x{:08X}", count, cause_code, vm.cpu.csr.sepc, stvec);
                        count += 1;
                        continue;
                    }
                }
        ...
        if !entered_kernel_va && vm.cpu.pc >= 0xC0000000 {
            ...
            eprintln!("[{}] *** ENTERED KERNEL VA SPACE: PC=0x{:08X} ***", count, vm.cpu.pc);
        }
        ...
        if entered_kernel_va && !returned_to_zero && vm.cpu.pc < 0x100 {
            ...
            trace_mode = true;
            eprintln!("[{}] *** RETURNED TO LOW ADDR: PC=0x{:08X} RA=0x{:08X} SP=0x{:08X} ***",
                count, vm.cpu.pc, vm.cpu.x[1], vm.cpu.x[2]);
            eprintln!("[{}]   satp=0x{:08X} stvec=0x{:08X} sscratch=0x{:08X}",
