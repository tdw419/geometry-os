...
fn main() {
    ...
    eprintln!("[dtb] DTB at PA 0x{:08X}, VA 0x{:08X}", dtb_addr as u32, dtb_va);
    ...
    eprintln!("[dtb] _dtb_early_va (PA 0x00801008) = 0x{:08X}", dtb_early_va);
    eprintln!("[dtb] _dtb_early_pa (PA 0x0080100C) = 0x{:08X}", dtb_early_pa);
    ...
    eprintln!("[dtb] Magic at _dtb_early_va (0x{:08X}): 0x{:08X}", dtb_early_va, dtb_early_va_magic);

    // Check DTB magic at _dtb_early_pa
    let dtb_early_pa_magic = vm.bus.read_word(dtb_early_pa as u64).unwrap_or(0);
    eprintln!("[dtb] Magic at _dtb_early_pa (0x{:08X}): 0x{:08X}", dtb_early_pa, dtb_early_pa_magic);
    ...
    eprintln!("[dtb] totalsize = 0x{:08X} ({} bytes)", totalsize, totalsize);
    ...
    for offset in 0..totalsize.saturating_sub(search.len() as u32) {
        ...
        for (j, &byte) in search.iter().enumerate() {
            let b = vm.bus.read_byte(dtb_addr + offset as u64 + j as u64).unwrap_or(0);
            if b != byte { match_all = false; break; }
        }
        if match_all {
            eprintln!("[dtb]   Found at DTB offset 0x{:04X}", offset);
            // Read the 4-byte value that follows (u32 big-endian)
            let val_bytes: Vec<u8> = (0..4)
                .map(|j| vm.bus.read_byte(dtb_addr + offset as u64 + search.len() as u64 + j as u64).unwrap_or(0))
                .collect();
