...
fn main() {
    ...
    while count < max_instructions {
        ...
        if vm.cpu.pc == 0x10EA && vm.cpu.privilege == geometry_os::riscv::cpu::Privilege::Supervisor && !saw_setup_vm_call {
            saw_setup_vm_call = true;
            eprintln!("[diag] setup_vm CALL at count={}: PC=0x{:08X} RA=0x{:08X}", count, vm.cpu.pc, vm.cpu.x[1]);
        }
        
        // Watch for setup_vm return (PC == RA from the call)
        if saw_setup_vm_call && !saw_setup_vm_ret && vm.cpu.pc == 0x10EE && vm.cpu.privilege == geometry_os::riscv::cpu::Privilege::Supervisor {
            saw_setup_vm_ret = true;
            eprintln!("[diag] setup_vm RETURN at count={}: PC=0x{:08X} a0=0x{:08X}", count, vm.cpu.pc, vm.cpu.x[10]);
