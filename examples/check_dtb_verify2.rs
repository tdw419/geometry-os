        ...
        if &blob[i..i+3] == reg_str {
            ...
            if i + 3 < blob.len() && blob[i+3] == 0 {
                ...
                let prop_len = u32::from_be_bytes([
                    blob[len_offset], blob[len_offset+1],
                    blob[len_offset+2], blob[len_offset+3]
                ]) as usize;
                ...
                if prop_len >= 8 {
                    let addr = u32::from_be_bytes([
                        blob[data_start], blob[data_start+1],
                        blob[data_start+2], blob[data_start+3]
                    ]);
                    let size = u32::from_be_bytes([
                        blob[data_start+4], blob[data_start+5],
                        blob[data_start+6], blob[data_start+7]
                    ]);
                    eprintln!("  Memory: base=0x{:08X} size=0x{:08X} ({}MB)", addr, size, size / (1024*1024));
                }
                break;
            }
        }
        ...
        if &blob[i..i+6] == chosen_str && (i + 6 >= blob.len() || blob[i+6] == 0) {
            ...
            for j in i..blob.len() - 8 {
                if &blob[j..j+8] == bootargs_str && blob[j+8] == 0 {
                    ...
                    let data_start = len_offset + 4;
                    let bootargs = String::from_utf8_lossy(&blob[data_start..data_start+prop_len]);
