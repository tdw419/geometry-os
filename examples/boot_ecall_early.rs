...
fn main() {
    ...
    // Boot to 200K
    let (mut vm, _) = RiscvVm::boot_linux(&kernel_image, initramfs.as_deref(), 256, 200_000, bootargs).unwrap();
    ...
    println!("  sstatus (mstatus & mask): 0x{:08X}", vm.cpu.csr.mstatus & 0x0007E1DE);
