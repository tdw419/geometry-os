...
fn main() {
    ...
    while count < max_instructions {
        ...
        if cur_satap != 0 && count < 177270 {
            ...
            eprintln!("[diag] SATAP becoming non-zero at count={}: 0x{:08X}", count, cur_satap);
            ...
            eprintln!("[diag] Non-zero words in early_pg_dir (0x802000-0x803000): {}", nonzero_in_pt);
            ...
            eprintln!("[diag] Non-zero words in trampoline_pg_dir (0x1484000-0x1485000): {}", nonzero_in_tramp);
            break;
        }
    ...
    if count >= 177270 {
        eprintln!("[diag] Reached max count without SATAP change. PC=0x{:08X}", vm.cpu.pc);
    }
}
