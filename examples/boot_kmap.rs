...
fn main() {
    ...
    println!("Boot: {} instructions, PC: 0x{:08X}", result.instructions, vm.cpu.pc);
    ...
    println!("\nPatched instruction at PA 0x{:08X}: 0x{:04X} (expect 0x0001 = C.NOP)", setup_vm_phys_addr_store, inst);
    
    let setup_vm_va_pa_offset_store: u64 = 0x00404968;
    let inst2 = vm.bus.read_half(setup_vm_va_pa_offset_store).unwrap_or(0);
    println!("Patched instruction at PA 0x{:08X}: 0x{:04X} (expect 0x0001 = C.NOP)", setup_vm_va_pa_offset_store, inst2);
    ...
    println!("\n--- After setup_vm ({} instructions) ---", result2.instructions);
    ...
    for i in 768..775 {
        let entry = vm2.bus.read_word(early_pg_dir_pa + (i as u64) * 4).unwrap_or(0);
        if entry != 0 {
            let ppn = (entry >> 10) & 0x3FFFFF;
            let is_leaf = (entry & 0xE) != 0;
            let ppn_hi = (ppn >> 10) & 0xFFF;
            println!("  L1[{}] = 0x{:08X} PPN=0x{:06X} ppn_hi={} leaf={}", i, entry, ppn, ppn_hi, is_leaf);
        }
    }
    ...
    if is_leaf_768 {
        ...
        println!("\n  L1[768] maps VA 0xC0000000 -> PA 0x{:08X} (ppn_hi={})", pa, ppn_hi);
