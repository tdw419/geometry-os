...
fn main() {
    ...
    while count < max {
        ...
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == Privilege::Machine {
            ...
            eprintln!("[TRAP #{}] mcause=0x{:08X} code={} mpp={} mepc=0x{:08X} RA=0x{:08X} at count={}",
        ...
        if trace_mode && vm.cpu.pc < 0xC0000000 || vm.cpu.pc > 0xD0000000 {
            eprintln!("[{}] PC OUT OF RANGE: 0x{:08X} RA=0x{:08X} SP=0x{:08X} priv={:?}",
        ...
        if trace_mode {
            let ra = vm.cpu.x[1];
            if ra != last_ra {
                eprintln!("[{}] RA: 0x{:08X} -> 0x{:08X} PC=0x{:08X}", count, last_ra, ra, vm.cpu.pc);
                last_ra = ra;
            }
        }
        
        if matches!(result, StepResult::FetchFault | StepResult::LoadFault | StepResult::StoreFault) {
            eprintln!("[FAULT] {:?} at count={} PC=0x{:08X}", result, count, vm.cpu.pc);
            break;
        }
    }
