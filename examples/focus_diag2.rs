...
fn main() {
    ...
    println!("Last non-zero word at: {}", asm.pixels.iter().rposition(|&w| w != 0).unwrap_or(0));
    ...
    let keys: Vec<u32> = vec![b'/' as u32, b'f' as u32, b'o' as u32, b'c' as u32,
    ...
    for (ki, &key) in keys.iter().enumerate() {
        ...
        if vm.yielded { vm.yielded = false; }
        ...
        println!("Key {} (0x{:02X}='{}'): halted={}, PC={}, cycles={}, RAM[0x7821]={}", 
                 ki, key, if key >= 32 && key < 127 { key as u8 as char } else { '?' },
                 vm.halted, vm.pc, cycles, vm.ram[0x7821]);
        
        if vm.halted {
            // Check what's at PC - is it zero?
            println!("  RAM[PC]={}, RAM[PC+1]={}, RAM[PC+2]={}", 
