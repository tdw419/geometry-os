fn main() {
    ...
    let mut found = false;
    let mut prev_pcs: std::collections::VecDeque<u32> = std::collections::VecDeque::with_capacity(20);
        ...
        if count >= start_watch && count % 1000 == 0 {
            ...
            if (satp >> 31) & 1 != 0 {
                ...
                if is_valid != last_valid {
                    eprintln!("[!] count={} PTE changed: 0x{:08X} -> 0x{:08X} V={} PC=0x{:08X}",
    ...
    if !found {
        ...
        let satp = vm.cpu.csr.read(csr::SATP);
        let pte = vm.bus.read_word(watch_addr).unwrap_or(0);
        eprintln!("Final: satp=0x{:08X} L1[770]=0x{:08X} V={}", satp, pte, pte & 1);
    }
}
