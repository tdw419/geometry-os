fn main() {
    // More targeted: run to 16,980,000 (just before corruption),
    // then check L1 entries, then run 1000 more and check again.
    ...
    // Run to just before corruption
    let target1 = 16_979_500u64;
    let mut count: u64 = 0;
    ...
    // Run 200 more instructions, checking every instruction
    for _ in 0..200 {
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == Privilege::Machine {
            let mcause = vm.cpu.csr.mcause;
            let cause_code = mcause & !(1u32 << 31);
        ...
        if pte != pte_before {
            eprintln!("[!] count={} PC=0x{:08X} step={:?}", count, prev_pc, step_result);
            eprintln!("  L1[770]: 0x{:08X} -> 0x{:08X}", pte_before, pte);
    ...
    eprintln!("After: count={} PC=0x{:08X} L1[770]=0x{:08X}", count, vm.cpu.pc, pte_final);
}
