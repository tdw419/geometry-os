...
fn main() {
    ...
    while count < 180_000 {
        ...
        if vm.cpu.pc == 0xC003F8AA && !in_prb_reserve {
            ...
            eprintln!("[CALL {}] prb_reserve at count={}, ra=0x{:08X}, sp=0x{:08X}", 
        ...
        if vm.cpu.pc == 0xC003F9E6 && in_prb_reserve {
            eprintln!("[RET {}] prb_reserve returning at count={}, ra=0x{:08X}",
                count, count, vm.cpu.x[1]);
            if vm.cpu.x[1] != prb_entry_ra {
                eprintln!("  *** RA CHANGED! Entry ra=0x{:08X}, now ra=0x{:08X} ***",
                    prb_entry_ra, vm.cpu.x[1]);
                ...
                if saved_ra_pa >= 0xC0000000 {
                    ...
                    eprintln!("  Stack[sp-4] = VA 0x{:08X} PA 0x{:08X} = 0x{:08X}", 
