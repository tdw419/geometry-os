fn main() {
    ...
    let (mut vm, _) = RiscvVm::boot_linux(&kernel_image, initramfs.as_deref(), 256, 500_000, bootargs).unwrap();
    
    let satp = vm.cpu.csr.read(geometry_os::riscv::csr::SATP);
    let mode = satp >> 31;
    let asid = (satp >> 22) & 0x1FF;
    let ppn = satp & 0x3FFFFF;
    println!("SATP: 0x{:08X} mode={} asid={} ppn=0x{:X} (phys=0x{:08X})", satp, mode, asid, ppn, ppn << 12);
    ...
    match mmu::translate(fault_va, mmu::AccessType::Fetch, geometry_os::riscv::cpu::Privilege::Supervisor, false, false, satp, &mut vm.bus, &mut tlb) {
    ...
    match mmu::translate(kernel_va, mmu::AccessType::Fetch, geometry_os::riscv::cpu::Privilege::Supervisor, false, false, satp, &mut vm.bus, &mut tlb) {
        mmu::TranslateResult::Ok(pa) => println!("  -> OK, PA=0x{:08X}", pa),
        other => println!("  -> {:?}", other),
    }
    ...
    for i in 0..16 {
        ...
        println!("  L1[{:2}]: 0x{:08X} valid={} leaf={} ppn=0x{:05X} flags={:03b}", 
            i, pte, valid, is_leaf, pte_ppn, pte & 0xE);
    }
