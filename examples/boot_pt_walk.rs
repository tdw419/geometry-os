fn main() {
    ...
    match mmu::translate(fault_va, mmu::AccessType::Fetch, false, satp, &vm.bus, &mut tlb) {
    ...
    match mmu::translate(kernel_va, mmu::AccessType::Fetch, false, satp, &vm.bus, &mut tlb) {
        mmu::TranslateResult::Ok(pa) => println!("  -> OK, PA=0x{:08X}", pa),
        other => println!("  -> {:?}", other),
    }
