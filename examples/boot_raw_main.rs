...
fn main() {
    ...
    eprintln!("Setup done: entry=0x{:08X} fw_addr=0x{:X} dtb=0x{:X}", entry, fw_addr, dtb_addr);
    eprintln!("Initial: PC=0x{:08X} priv={:?} SATP=0x{:08X}", vm.cpu.pc, vm.cpu.privilege, vm.cpu.csr.satp);
    ...
    while count < 10_000_000 {
        ...
        if vm.cpu.csr.satp != last_satp {
            ...
            eprintln!("[{}] SATP -> 0x{:08X} (ppn=0x{:X} pa=0x{:X}) PC=0x{:08X}",
                count, vm.cpu.csr.satp, ppn, ppn as u64 * 4096, vm.cpu.pc);
        ...
        match result {
            ...
            StepResult::FetchFault | StepResult::LoadFault | StepResult::StoreFault => {
                if count < 200 {
                    eprintln!("[{}] {:?} at PC=0x{:08X} priv={:?}", count, result, vm.cpu.pc, vm.cpu.privilege);
                }
            }
            StepResult::Ecall => {
                sbi_count += 1;
        ...
        if count % 1_000_000 == 0 {
            eprintln!("[{}] PC=0x{:08X} priv={:?} UART={}", count, vm.cpu.pc, vm.cpu.privilege, uart_bytes);
        }
    }
    
    eprintln!("\nFinal: {} steps, {} SBI calls, {} UART bytes", count, sbi_count, uart_bytes);
    let uart_str: String = vm.bus.uart.tx_buf.iter().chain(vm.bus.sbi.console_output.iter()).map(|&b| b as char).collect();
