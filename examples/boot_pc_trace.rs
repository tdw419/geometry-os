...
fn main() {
    ...
    while count < max {
        ...
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == geometry_os::riscv::cpu::Privilege::Machine {
            ...
            if cause_code == 11 {
                ...
                sbi_count += 1;
                if sbi_count <= 5 {
                    eprintln!("[sbi] SBI call #{sbi_count}: a7=0x{:08X} a6=0x{:08X} a0=0x{:08X}",
        ...
        if count % 500_000 == 0 && count > 0 {
            eprintln!("[sample {}] PC=0x{:08X} prev_pc=0x{:08X} sbi={} faults={}",
    ...
    eprintln!("[done] count={} sbi={} faults={} uart={}", 
        count, sbi_count, fault_count, vm.bus.uart.tx_buf.len());
    if !vm.bus.uart.tx_buf.is_empty() {
        let s = String::from_utf8_lossy(&vm.bus.uart.tx_buf);
        eprintln!("[UART] {}", &s[..s.len().min(500)]);
    }
    eprintln!("[state] PC=0x{:08X} priv={:?} satp=0x{:08X}", vm.cpu.pc, vm.cpu.privilege, vm.cpu.csr.satp);
}
