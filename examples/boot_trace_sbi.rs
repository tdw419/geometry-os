...
fn main() {
    ...
    let (mut vm, _) = RiscvVm::boot_linux(
        &kernel, initramfs.as_deref(), 256, 3_000_000, bootargs,
    ).unwrap();

    println!("After boot_linux: PC=0x{:08X} priv={:?}", vm.cpu.pc, vm.cpu.privilege);
    ...
    for i in 0u64..2_000_000 {
        ...
        if i - last_progress >= 500_000 {
            last_progress = i;
            println!("[{}] PC=0x{:08X} priv={:?} SBI={}B stall={}", 
                     i, vm.cpu.pc, vm.cpu.privilege, new_len, stall);
        }
        
        // Hard stop if stalling too long
        if stall > 1_000_000 {
            println!("[{}] Hard stall at 0x{:08X} for {} instructions", i, vm.cpu.pc, stall);
            break;
        }
    }
