...
fn main() {
    ...
    for i in 0..300_000 {
        ...
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == geometry_os::riscv::cpu::Privilege::Machine {
            ...
            println!("[{}] M-TRAP: cause={} mepc={:#010x}", i, cause_code, vm.cpu.csr.mepc);
        }
        ...
        if vm.cpu.pc < 0x100 && pc >= 0x100 {
            ...
            println!("  RA={:#010x} SP={:#010x} GP={:#010x} TP={:#010x}",
                vm.cpu.x[1], vm.cpu.x[2], vm.cpu.x[3], vm.cpu.x[4]);
            println!("  T0={:#010x} T1={:#010x} T2={:#010x}",
                vm.cpu.x[5], vm.cpu.x[6], vm.cpu.x[7]);
            println!("  A0={:#010x} A1={:#010x} A2={:#010x} A3={:#010x}",
                vm.cpu.x[10], vm.cpu.x[11], vm.cpu.x[12], vm.cpu.x[13]);
            println!("  A4={:#010x} A5={:#010x} A6={:#010x} A7={:#010x}",
                vm.cpu.x[14], vm.cpu.x[15], vm.cpu.x[16], vm.cpu.x[17]);
            println!("  satp={:#010x} stvec={:#010x}", vm.cpu.csr.satp, vm.cpu.csr.stvec);
            println!("  sepc={:#010x} scause={:#010x} stval={:#010x}",
                vm.cpu.csr.sepc, vm.cpu.csr.scause, vm.cpu.csr.stval);
            println!("  mepc={:#010x} mcause={:#010x}",
