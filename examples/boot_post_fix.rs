...
fn main() {
    ...
    while count < max {
        ...
        if pc == 0xC040AF6E {
            eprintln!("[{}] C.LUI a5 at PC=0x{:08X}: a5_before=0x{:08X}", 
                count, pc, vm.cpu.x[15]);
        }
        if pc == 0xC040AF72 {
            eprintln!("[{}] After AND: s8=0x{:08X} (should be 0xCFDB6000)", 
                count, vm.cpu.x[24]);
        }
        
        // Track pcpu_alloc_first_chunk
        if pc == 0xC040AF6C {
            eprintln!("[{}] pcpu_alloc_first_chunk(a0=0x{:08X}, a1=0x{:08X})", 
                count, vm.cpu.x[10], vm.cpu.x[11]);
        }
        
        // Track the start_offset branch
        if pc == 0xC040B060 {
            eprintln!("[{}] Reading start_offset from chunk: val=0x{:08X}", 
                count, vm.cpu.x[24]);  // s4 will be loaded here
        }
        ...
        // Track panic
        if pc == 0xC000252E {
            eprintln!("[{}] PANIC called! a0=0x{:08X} (string ptr)", count, vm.cpu.x[10]);
        ...
        if vm.cpu.csr.scause != 0 && vm.cpu.csr.scause != 8 && vm.cpu.csr.scause != 9 {
            eprintln!("[{}] UNEXPECTED scause={} stval=0x{:08X} PC=0x{:08X}", 
                count, vm.cpu.csr.scause, vm.cpu.csr.stval, pc);
            break;
        }
    }
    
    eprintln!("Final: {} instructions, scause={}", count, vm.cpu.csr.scause);
}
