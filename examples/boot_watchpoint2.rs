fn main() {
    ...
    while count < max_instr {
        ...
        if vm.bus.write_watch_hit {
            ...
            if should_report {
                eprintln!("[WATCH] count={} hit#{} PC=0x{:08X} val=0x{:08X} V={}->{} PTE=0x{:08X}",
                    count, hit_count, pc_before, vm.bus.write_watch_val,
                    last_pte_v.unwrap(), is_v, pte);
                eprintln!("  a0=0x{:08X} a1=0x{:08X} a2=0x{:08X} sp=0x{:08X} ra=0x{:08X}",
    ...
    eprintln!("[DONE] count={} hits={} PC=0x{:08X}", count, hit_count, vm.cpu.pc);
    let pte = vm.bus.read_word(0x1002C08).unwrap_or(0);
