...
fn main() {
    ...
    while count < max {
        ...
        if vm.cpu.pc == last_pc {
            spin_count += 1;
            if spin_count > 10000 {
                println!("\nSPIN LOOP at PC=0x{:08X} after {} instructions", vm.cpu.pc, count);
                break;
            }
        } else {
            spin_count = 0;
        ...
        if count % report_interval == 0 {
            println!("After {} instrs: PC=0x{:08X} SP=0x{:08X}", 
    ...
    println!("PC: 0x{:08X}", vm.cpu.pc);
    println!("UART output ({} bytes): {}", uart_bytes.len(), &uart_str[..uart_str.len().min(500)]);
}
