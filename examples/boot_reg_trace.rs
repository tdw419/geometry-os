...
fn main() {
    ...
    while count < max_instr {
        ...
        if vm.cpu.pc < 0x10000 && pc_before >= 0xC0000000 {
            ...
            println!("{:>6}  {:>10}  {:>10}  x1(ra)   a0(x10)  a1(x11)  a2(x12)  a7(x17)",
                     "step", "PC", "instr");
            for (step, pc, word, regs) in &history {
                println!("{:>6}  0x{:08X}  0x{:08X}  0x{:08X}  0x{:08X}  0x{:08X}  0x{:08X}  0x{:08X}",
    ...
    if !out.is_empty() {
        println!("\nUART ({} bytes): {}", out.len(), String::from_utf8_lossy(&out));
    }
}
