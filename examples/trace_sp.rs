...
fn main() {
    ...
    while count < 200_000 {
        ...
        if vm.cpu.pc == 0xC003B5BC {
            eprintln!("[ENTRY] vprintk_store at count={}, sp=0x{:08X}", count, vm.cpu.x[2]);
        }
        ...
        if vm.cpu.pc == 0xC003B5C2 {
            eprintln!("[POST-ALLOC] sp=0x{:08X}, s0 should be sp+144=0x{:08X}, actual s0=0x{:08X}",
                vm.cpu.x[2], vm.cpu.x[2].wrapping_add(144), vm.cpu.x[8]);
        }

        // First call to prb_reserve
        if vm.cpu.pc == 0xC003B716 {
            eprintln!("[1ST PRB] count={}, sp=0x{:08X}, s0=0x{:08X}, a0=0x{:08X}", 
                count, vm.cpu.x[2], vm.cpu.x[8], vm.cpu.x[10]);
        }

        // Second call to prb_reserve
        if vm.cpu.pc == 0xC003B754 {
            eprintln!("[2ND PRB] count={}, sp=0x{:08X}, s0=0x{:08X}, a0=0x{:08X}", 
                count, vm.cpu.x[2], vm.cpu.x[8], vm.cpu.x[10]);
            eprintln!("  a0 = s0-92 = 0x{:08X}-92 = 0x{:08X}", vm.cpu.x[8], vm.cpu.x[8].wrapping_sub(92));
        }
