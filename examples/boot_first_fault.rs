...
fn main() {
    ...
    while count < max {
        ...
        if cur_satp != last_satp {
            ...
            eprintln!("[SATP] 0x{:08X} (pg_dir PA 0x{:08X}) at count={}",
        ...
        if !first_fault_logged && vm.cpu.pc == fw_addr_u32 {
            ...
            if mpp == 1 && (cause_code == 5 || cause_code == 7 || 
                ...
                eprintln!("  cause={} ({}), mtval=0x{:08X}, mepc=0x{:08X}",
                ...
                eprintln!("  PT walk: L1[{}] at PA 0x{:08X} = 0x{:08X} (valid={} leaf={} ppn=0x{:06X})",
                    vpn1, l1_addr, l1_entry,
                    (l1_entry & 1) != 0, (l1_entry & 0xE) != 0,
                ...
                if (l1_entry & 1) != 0 && (l1_entry & 0xE) == 0 {
                    ...
                    eprintln!("           L2[{}] at PA 0x{:08X} = 0x{:08X} (valid={} ppn=0x{:06X})",
                        vpn0, l2_addr, l2_entry,
                        (l2_entry & 1) != 0, (l2_entry >> 10) & 0x3FFFFF);
                }
                ...
                if mem_regions_ptr >= 0xC0000000 {
                    ...
                    for ri in 0..mem_cnt.min(8) {
                        ...
                        eprintln!("    memory[{}]: base=0x{:08X} size=0x{:08X} (end=0x{:08X})",
                            ri, b, s, b + s);
                    }
                }
        ...
        if count % 5_000_000 == 0 {
            eprintln!("[progress] {}M, PC=0x{:08X}, ECALLs={}",
                count / 1_000_000, vm.cpu.pc, vm.cpu.ecall_count);
        }
    }

    eprintln!("[done] {} instructions, first_fault_logged={}", count, first_fault_logged);
}
