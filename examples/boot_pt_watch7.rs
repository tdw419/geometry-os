fn main() {
    ...
    while count < target1 {
        ...
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == Privilege::Machine {
            ...
            if cause_code == csr::CAUSE_ECALL_S || cause_code == csr::CAUSE_ECALL_M {
                let result = vm.bus.sbi.handle_ecall(vm.cpu.x[17], vm.cpu.x[16], vm.cpu.x[10], vm.cpu.x[11], vm.cpu.x[12], vm.cpu.x[13], vm.cpu.x[14], vm.cpu.x[15], &mut vm.bus.uart, &mut vm.bus.clint);
                if let Some((a0, a1)) = result { vm.cpu.x[10] = a0; vm.cpu.x[11] = a1; }
            } else if mpp != 3 {
                let stvec = vm.cpu.csr.stvec & !0x3u32;
                if stvec != 0 {
                    vm.cpu.csr.sepc = vm.cpu.csr.mepc; vm.cpu.csr.scause = mcause; vm.cpu.csr.stval = vm.cpu.csr.mtval;
                    ...
                    vm.cpu.pc = stvec; vm.cpu.privilege = Privilege::Supervisor; vm.cpu.tlb.flush_all();
                    count += 1; continue;
                }
            }
            vm.cpu.csr.mepc = vm.cpu.csr.mepc.wrapping_add(4);
        }
        let _ = vm.step(); count += 1;
    }

    let pte_before = vm.bus.read_word(watch_addr).unwrap_or(0);
    eprintln!("Before: count={} PC=0x{:08X} L1[770]=0x{:08X}", count, vm.cpu.pc, pte_before);
    ...
    for _ in 0..500 {
        ...
        if pte != pte_before {
            eprintln!("[!] count={} PC=0x{:08X} L1[770]: 0x{:08X} -> 0x{:08X}", count, vm.cpu.pc, pte_before, pte);
            eprintln!("  a0=0x{:08X} a1=0x{:08X} a2=0x{:08X} a3=0x{:08X} a4=0x{:08X}",
    ...
    eprintln!("Final: count={} PC=0x{:08X} L1[770]=0x{:08X}", count, vm.cpu.pc, pte_final);
}
