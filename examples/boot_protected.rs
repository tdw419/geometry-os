...
fn main() {
    ...
    println!("BEFORE boot: initial_boot_params (word)=0x{:08X}", ibp_before);
    println!("BEFORE boot: initial_boot_params (byte0)=0x{:02X}", ibp_byte0);
    
    // Check how many protected_addrs we have
    println!("Protected addresses: {} entries", vm.bus.protected_addrs.len());
    ...
    println!("  initial_boot_params (word)=0x{:08X} (expect DTB PA 0x{:08X})", ibp_after_word, dtb_addr as u32);
    ...
    println!("  initial_boot_params (half_lo)=0x{:04X} (half_hi)=0x{:04X}", ibp_via_half, ibp_via_half_hi);
    
    // Check DTB readability
    let dtb_magic = vm.bus.read_word(dtb_addr).unwrap_or(0);
    println!("\n  DTB magic at PA 0x{:08X}: 0x{:08X} (expect 0xD00DFEED)", dtb_addr, dtb_magic);
