...
fn main() {
    ...
    while count < max_count {
        ...
        if new_ecall_count > pre_ecall_count {
            eprintln!("[ecall] count={}: a7=0x{:X} a6=0x{:X} a0=0x{:X} a1=0x{:X} ret_a0=0x{:X} ret_a1=0x{:X}",
                count, vm.cpu.x[17], vm.cpu.x[16], vm.cpu.x[10], vm.cpu.x[11],
    ...
    eprintln!("\nTotal: {} instructions, {} ECALLs", count, vm.cpu.ecall_count);
