...
fn main() {
    ...
    while count < max_instructions {
        ...
        if in_setup_vm && vm.bus.write_watch_hit {
            pt_write_count += 1;
            writes_to_pt.push((count, vm.bus.write_watch_addr.unwrap(), vm.bus.write_watch_val));
            if pt_write_count <= 5 {
                eprintln!("[diag] PT WRITE #{} at count={}: addr=0x{:08X} val=0x{:08X}",
                    pt_write_count, count, vm.bus.write_watch_addr.unwrap(), vm.bus.write_watch_val);
            }
        ...
        if in_setup_vm && vm.cpu.pc == 0x10EE {
            eprintln!("[diag] Returned from setup_vm at count={}, PT writes={}", count, pt_write_count);
            // Show all writes
