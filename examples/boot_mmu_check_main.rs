...
fn main() {
    ...
    eprintln!("SATP=0x{:08X} ppn=0x{:X} pgdir_pa=0x{:X}", satp, ppn, pgdir_pa);
    ...
    for l1_idx in [768, 769, 770, 771, 772, 800, 801, 900, 901, 1020, 1021] {
        ...
        eprintln!("L1[{}] VA 0x{:08X}: PTE=0x{:08X} V={}R={}W={}X={} PPN=0x{:X}->PA 0x{:X}",
    ...
    for pa in [0x0u64, 0x1000, 0x10000, 0x100000, 0x200000, 0x400000, 0x800000] {
