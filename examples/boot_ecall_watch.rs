...
fn main() {
    ...
    for count in 0..max {
        ...
        if sbi_len > 0 && count % 10000 == 0 {
            ...
            eprintln!("[{}] SBI output ({} bytes): {}", count, sbi_len, &s[..s.len().min(200)]);
        }
        ...
        if count == 178000 {
            ...
            eprintln!("[{}] State: ibp=0x{:08X} deva=0x{:08X} depa=0x{:08X} dtb_magic=0x{:08X} ecall={}",
                count, ibp, deva, depa, dtb_magic, vm.cpu.ecall_count);
            eprintln!("[{}] PC=0x{:08X} priv={:?}", count, vm.cpu.pc, vm.cpu.privilege);
        }
    }
    
    let sbi_out: String = vm.bus.sbi.console_output.iter().map(|&b| b as char).collect();
    eprintln!("SBI output ({} bytes): {}", vm.bus.sbi.console_output.len(), 
        if sbi_out.is_empty() { "(none)".to_string() } else { sbi_out[..sbi_out.len().min(500)].to_string() });
