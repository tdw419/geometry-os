...
fn main() {
    ...
    let initial_data: Vec<u8> = (0..64)
        ...
        .collect();
    let initial_str: String = initial_data.iter()
        .take_while(|&&b| b != 0)
        .map(|&b| b as char)
        .collect();
    ...
    while count < max_instr {
        ...
        if cur_satp != last_satp {
            eprintln!("[check] SATP: 0x{:08X} -> 0x{:08X} at count={}", last_satp, cur_satp, count);
    ...
    let post_data: Vec<u8> = (0..64)
        .map(|i| vm.bus.read_byte(fmt_pa + i as u64).unwrap_or(0))
        .collect();
    let post_str: String = post_data.iter()
        .take_while(|&&b| b != 0)
        .map(|&b| b as char)
        .collect();
    eprintln!("\n[check] Format string PA 0x{:08X} AFTER {} instructions:", fmt_pa, count);
    ...
    if initial_data != post_data {
        ...
        for i in 0..64 {
            if initial_data[i] != post_data[i] {
                eprintln!("[check]   offset +0x{:02X}: 0x{:02X} -> 0x{:02X}",
    ...
    eprintln!("\n[check] Checking page at PA 0x{:08X} (contains format string):", page_base);
    ...
    let page_words: Vec<u32> = (0..1024)
        ...
        .collect();
    let non_zero_entries: Vec<(usize, u32)> = page_words.iter().enumerate()
        .filter(|(_, &w)| w != 0)
        .map(|(i, &w)| (i, w))
        .collect();
    eprintln!("[check]   Non-zero words: {}/1024", non_zero_entries.len());
    for &(idx, val) in non_zero_entries.iter().take(20) {
        eprintln!("[check]   [0x{:03X}] = 0x{:08X}  V={} RWX={:03b} PPN=0x{:05X}",
            idx * 4, val, val & 1, (val >> 1) & 7, val >> 12);
    }
    ...
    for page_num in 0..16 {
        ...
        eprintln!("[check]   PA 0x{:08X} (page {}): first word = 0x{:08X}", pa, page_num, first_word);
    }
}
