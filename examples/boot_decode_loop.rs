fn main() {
    ...
    // Run to the loop
    for _ in 0..100_000 { let _ = vm.step(); }
    ...
    for i in 0..10 {
        ...
        println!("--- Step {} ---", i);
        println!("  PC=0x{:08X} insn=0x{:08X} priv={:?}", pc, insn, vm.cpu.privilege);
        println!("  x[10](a0)=0x{:08X} x[11](a1)=0x{:08X} x[12](a2)=0x{:08X}", vm.cpu.x[10], vm.cpu.x[11], vm.cpu.x[12]);
        println!("  x[13](a3)=0x{:08X} x[14](a4)=0x{:08X} x[15](a5)=0x{:08X}", vm.cpu.x[13], vm.cpu.x[14], vm.cpu.x[15]);
        ...
        match opcode {
            0x23 => { // S-type (store)
                ...
                println!("  STORE: sw x[{}], {}(x[{}]) -- store 0x{:08X} to 0x{:08X}", 
                    rs2, imm, rs1, vm.cpu.x[rs2], (vm.cpu.x[rs1] as i32 + imm) as u32);
            }
            0x13 => { // I-type (addi etc)
                let imm = ((insn as i32) >> 20) as i32;
                let op = match funct3 { 0 => "addi", _ => "op" };
                println!("  {}: x[{}] = x[{}] + {} = 0x{:08X}", op, rd, rs1, imm, 
