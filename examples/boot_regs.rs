    ...
    while count < max_instr {
        ...
        if mcause != prev_mcause && mcause != 0 {
            ...
            if !is_timer {
                ...
                println!("  mepc=0x{:08X} mtval=0x{:08X} mtvec=0x{:08X}", 
                    vm.cpu.csr.mepc, vm.cpu.csr.mtval, vm.cpu.csr.mtvec);
                println!("  stvec=0x{:08X} satp=0x{:08X} priv={:?}", 
                    vm.cpu.csr.stvec, vm.cpu.csr.satp, vm.cpu.privilege);
                println!("  medeleg=0x{:08X} mideleg=0x{:08X}", 
                ...
                if let Ok(inst) = vm.bus.read_word(mepc as u64) {
                    ...
                    println!("\n  Faulting instruction at mepc=0x{:08X}: 0x{:08X}", mepc, inst);
                    println!("  opcode=0x{:02X} rd=x{} funct3={} rs1=x{} rs2=x{} funct7=0x{:02X}", 
                    ...
                    if opcode == 0x23 {
                        ...
                        println!("  SW x{}, {}(x{}) -> addr = 0x{:X} + {} = 0x{:X}", 
                            rs2, imm, rs1, vm.cpu.x[rs1_usize], imm, store_addr);
                        println!("  x{} = 0x{:08X}, value to store (x{}) = 0x{:08X}", 
