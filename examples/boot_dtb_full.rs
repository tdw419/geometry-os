...
fn main() {
    ...
    println!("  magic:             0x{:08X} (expect 0xD00DFEED)", r32(&hdr, 0));
    ...
    for i in 0..8 {
        let addr = vm.bus.read_word(dtb_addr + mem_rsv_off + (i * 16) as u64).unwrap_or(0);
        let size = vm.bus.read_word(dtb_addr + mem_rsv_off + (i * 16 + 4) as u64).unwrap_or(0);
    ...
    println!("  Value: 0x{:08X} (expect DTB PA 0x{:08X})", ibp_val, dtb_addr as u32);
    ...
    println!("\nDTB readable via identity map (VA=PA 0x01579000): 0x{:08X}", dtb_via_va);
    
    // Check kernel_map values
    let km_pa = 0x00C7A098u64;
    println!("\nkernel_map at PA 0x{:08X}:", km_pa);
    println!("  page_offset:    0x{:08X}", vm.bus.read_word(km_pa).unwrap_or(0));
    println!("  virt_addr:      0x{:08X}", vm.bus.read_word(km_pa + 4).unwrap_or(0));
    println!("  virt_offset:    0x{:08X}", vm.bus.read_word(km_pa + 8).unwrap_or(0));
    println!("  phys_addr:      0x{:08X}", vm.bus.read_word(km_pa + 12).unwrap_or(0));
    println!("  size:           0x{:08X}", vm.bus.read_word(km_pa + 16).unwrap_or(0));
    println!("  va_pa_offset:   0x{:08X}", vm.bus.read_word(km_pa + 20).unwrap_or(0));
    println!("  va_kernel_pa_offset: 0x{:08X}", vm.bus.read_word(km_pa + 24).unwrap_or(0));
}
