...
fn main() {
    ...
    while count < max {
        ...
        // Identity mapping injection
        {
            ...
            if trampoline_patched && cur_satp != last_satp && cur_satp != 0 {
                eprintln!("[boot] SATP: 0x{:08X} -> 0x{:08X} at count={}", last_satp, cur_satp, count);
        ...
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == geometry_os::riscv::cpu::Privilege::Machine {
            ...
            if trap_count <= 10 {
                ...
                eprintln!("[TRAP] #{} cause={} mepc=0x{:08X} mpp={} at count={}",
        ...
        if fine_trace {
            ...
            if rel < 500 || (rel < 5000 && rel % 10 == 0) || (rel < 50000 && rel % 100 == 0) {
                ...
                eprintln!("[{:>6}] PC=0x{:08X} satp=0x{:08X} priv={}", rel, pc, satp, prv);
            }
            ...
            if vm.cpu.csr.satp != 0 && vm.cpu.pc < 0x1000 && count > fine_start + 100 {
                ...
                eprintln!("[!] LOW PC at +{}: PC=0x{:08X} inst=0x{:08X}", count - fine_start, vm.cpu.pc, inst);
    ...
    eprintln!("Final PC: 0x{:08X} SATP: 0x{:08X}", vm.cpu.pc, vm.cpu.csr.satp);
}
