fn main() {
    ...
    eprintln!("At 11.765M: table=0x{:08X}", vm.bus.read_word(table_addr).unwrap_or(0));
    ...
    for i in 11_765_000..11_770_000u64 {
        ...
        if new_val != last_table_val {
            eprintln!("[TABLE CHANGE] count={} PC=0x{:08X} 0x{:08X}->0x{:08X}", 
                i, pc_before, last_table_val, new_val);
            // Print registers to see memmove args
            eprintln!("  a0(dst)=0x{:X} a1(src)=0x{:X} a2(len)=0x{:X} a3=0x{:X} a4=0x{:X}",
            ...
            for off in [-8i32, -4, 0, 4, 8] {
                ...
                eprintln!("  [0x{:08X}] = 0x{:08X}", a as u32, vm.bus.read_word(a).unwrap_or(0));
            }
    ...
    eprintln!("DONE. table=0x{:08X}", vm.bus.read_word(table_addr).unwrap_or(0));
}
