...
fn main() {
    ...
    while count < max_count {
        ...
        // SATP change handling (same as before)
        {
        ...
        // Handle M-mode traps at fw_addr (for non-SBI traps)
        if vm.cpu.pc == fw_addr_u32 && vm.cpu.privilege == Privilege::Machine {
            ...
            if is_interrupt {
                ...
                // ECALL_S handled by execute_system, shouldn't reach here
                vm.cpu.csr.mepc = vm.cpu.csr.mepc.wrapping_add(4);
        ...
        // Save pre-step state
        let pre_ecall_count = vm.cpu.ecall_count;

        vm.bus.tick_clint();
        vm.bus.sync_mip(&mut vm.cpu.csr.mip);
        let step_result = vm.step();

        // Check if ECALL happened during this step
        let new_ecall_count = vm.cpu.ecall_count;
        if new_ecall_count > pre_ecall_count {
            eprintln!("[ecall] count={}: ECALL #{} at PC=0x{:08X} a7=0x{:X} a6=0x{:X} a0=0x{:X} priv={:?} uart_tx={}",
                count, new_ecall_count, vm.cpu.pc, vm.cpu.x[17], vm.cpu.x[16], vm.cpu.x[10],
                vm.cpu.privilege, vm.bus.uart.tx_buf.len());
        }
