...
fn main() {
    ...
    if vm.halted { println!("HALTED before Enter"); return; }
    ...
    while !vm.yielded && !vm.halted && step < 5000 {
        ...
        trace.push(format!("{:04}: {} (r0={}, r1={}, r5={}, r20={}, r31={})", 
                          pc, mnem, vm.regs[0], vm.regs[1], vm.regs[5], vm.regs[20], vm.regs[31]));
        vm.step();
        step += 1;
    }
    
    println!("Traced {} steps, halted={}, yielded={}, final PC={}", step, vm.halted, vm.yielded, vm.pc);
    println!("RAM[0x7821] = {}", vm.ram[0x7821]);
    
    // Print last 40 trace entries
    let start = if trace.len() > 40 { trace.len() - 40 } else { 0 };
