...
fn main() {
    ...
    while step_count < 500_000 {
        ...
        if vm.cpu.csr.scause != 0 {
            ...
            eprintln!("  saved_ra=0x{:08X} (caller of pcpu_block_update_hint_alloc)", saved_ra);
            eprintln!("  a0=0x{:08X} a1=0x{:08X} a2=0x{:08X}", vm.cpu.x[10], vm.cpu.x[11], vm.cpu.x[12]);
            eprintln!("  s3(chunk)=0x{:08X} s4(block_idx)={}", vm.cpu.x[19], vm.cpu.x[20]);
            ...
            if s3 >= 0xC0000000 && s3 < 0xD0000000 {
                ...
                eprintln!("    free_bytes=0x{:08X} ({})", free_bytes, free_bytes as i32);
