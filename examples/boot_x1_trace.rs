...
fn main() {
    ...
    for i in 0..25 {
        ...
        let x1_changed = if vm.cpu.x[1] != old_x1 { format!(" x1: {:08X} -> {:08X}", old_x1, vm.cpu.x[1]) } else { String::new() };
        let x13_changed = if vm.cpu.x[13] != old_x13 { format!(" x13: {:08X} -> {:08X}", old_x13, vm.cpu.x[13]) } else { String::new() };
        eprintln!("{:6}: PC=0x{:08X} op={:?} -> 0x{:08X}{}{}", 
            count + i as u64, pc, last.op, vm.cpu.pc, x1_changed, x13_changed);
    }
}
