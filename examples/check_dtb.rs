...
fn parse_dtb(dtb: &[u8]) {
    ...
    eprintln!("[DTB] totalsize={} struct_off={} strings_off={} mem_rsvmap_off={} version={}",
    ...
    loop {
        ...
        eprintln!("  reserved: PA 0x{:08X} - 0x{:08X} ({} bytes)", addr, addr + size, size);
    }
    ...
    while pos < off_dt_strings {
        ...
        match token {
            0x00000001 => { // FDT_BEGIN_NODE
                let name_start = pos;
                while pos < dtb.len() && dtb[pos] != 0 { pos += 1; }
            ...
            0x00000003 => { // FDT_PROP
                ...
                let name_off = u32::from_be_bytes(dtb[pos+4..pos+8].try_into().unwrap()) as usize;
                pos += 8;
                let prop_data = &dtb[pos..pos+prop_len];
                pos += prop_len;
                pos = (pos + 3) & !3; // align to 4 bytes
                // Get property name
                let name_end = strings[name_off..].iter().position(|&b| b == 0).unwrap_or(0);
                let name = std::str::from_utf8(&strings[name_off..name_off+name_end]).unwrap_or("?");
                if name == "reg" || name == "device_type" || name == "compatible" {
                    if prop_len == 16 && name == "reg" {
                        ...
                        eprintln!("    PROP '{}': base=0x{:08X} size=0x{:08X} ({}MB)", name, a, s, s / (1024*1024));
                    } else {
                        let preview: String = prop_data.iter().take(40)
                            .map(|&b| if b >= 0x20 && b < 0x7f { b as char } else { '.' }).collect();
                        eprintln!("    PROP '{}': len={} data={:?}...", name, prop_len, preview);
                    }
                }
            }
...
fn main() {
    ...
    eprintln!("[DIAG] DTB at PA 0x{:08X}, size {} bytes", dtb_addr, dtb_blob.len());
    parse_dtb(&dtb_blob);
