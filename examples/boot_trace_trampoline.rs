...
fn main() {
    ...
    let mut entered_trampoline = false;
    let mut trampoline_entry_count = 0u32;
    let mut last_progress = 0u64;
    // Capture instruction trace around first trampoline re-entry
    let mut trace_buffer: Vec<(u64, u32, u32)> = Vec::new(); // (count, pc, instruction_word)
    ...
    while count < max_count {
        ...
        // Detect re-entry to trampoline (PC < 0x2000 after initial boot)
        if count > 200_000 && vm.cpu.pc < 0x2000 && vm.cpu.privilege == Privilege::Supervisor {
            ...
            trampoline_entry_count += 1;
        }

        // Capture trace around trampoline re-entry
        if tracing && trace_count < 200 {
            let instr_word = if vm.cpu.pc < 0x200000 {
                vm.bus.read_word(vm.cpu.pc as u64).unwrap_or(0)
            } else {
                // Virtual address - need to translate
                let vpn = vm.cpu.pc as u64;
                // Just store 0 for now, we mainly care about PC
                0
            };
            trace_buffer.push((count, vm.cpu.pc, instr_word));
            trace_count += 1;
            if trace_count >= 200 {
                tracing = false;
            }
    ...
    println!("ECALLs: {}, Traps: {}", ecall_count, trap_count);
    println!("Trampoline re-entries: {}", trampoline_entry_count);
