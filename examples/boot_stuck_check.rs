...
fn main() {
    ...
    for offset in -8..=8i32 {
        ...
        if let Ok(word) = vm.bus.read_word(addr) {
            let marker = if addr as u32 == stuck_pc { ">>>" } else { "   " };
    ...
    if !sbi.console_output.is_empty() {
        println!("First 200 chars: {}", String::from_utf8_lossy(&sbi.console_output[..sbi.console_output.len().min(200)]));
    }
    ...
    if !uart_out.is_empty() {
        println!("First 200 chars: {}", String::from_utf8_lossy(&uart_out[..uart_out.len().min(200)]));
    }
