...
fn main() {
    ...
    while count < max {
        ...
        if cur_satp != last_satp {
            ...
            eprintln!("  pgdir=0x{:08X} L1[0xFF]=0x{:08X} L1[0x300]=0x{:08X}", pgdir, l1_ff, l1_300);
            
            // Also check the write watchpoint
            if vm.bus.write_watch_hit {
                eprintln!("  WATCHPOINT HIT: wrote 0x{:08X} from PC=0x{:08X}", 
        ...
        if vm.bus.write_watch_hit {
            eprintln!("  WP HIT at count={}: wrote 0x{:08X} from PC=0x{:08X} priv={:?}", 
        ...
        match result {
            StepResult::FetchFault | StepResult::LoadFault | StepResult::StoreFault => {
                if count > 177_000 {
                    eprintln!("FAULT at count={}: PC=0x{:08X} result={:?} scause=0x{:08X} stval=0x{:08X}", 
