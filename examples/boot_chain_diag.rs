...
fn main() {
    ...
    for count in 0..5_000_000u64 {
        ...
        // Phase 1
        if !trampoline_patched && pc == 0x10EE && vm.cpu.privilege == Privilege::Supervisor && satp == 0 {
            let identity_pte: u32 = 0xCF;
            let l1_entries: &[u32] = &[0, 2, 4, 5, 6, 8, 10];
            for &idx in l1_entries {
                let pte = identity_pte | (idx << 20);
                vm.bus.write_word(0x01484000u64 + (idx * 4) as u64, pte).ok();
                vm.bus.write_word(0x00802000u64 + (idx * 4) as u64, pte).ok();
            }
        ...
        if trampoline_patched && satp != last_satp && satp != 0 {
            ...
            if mode == 1 {
                ...
                if !patched {
                    ...
                    eprintln!("[diag] Phase 2 injected into pg_dir=0x{:08X} at count={}", pg_dir, count);
                }
            }
        }
        last_satp = satp;
        ...
        if pc == fw_addr_u32 && vm.cpu.privilege == Privilege::Machine {
            ...
            if cause_code == 9 { // ECALL_S
                ...
                let a0 = vm.cpu.x[10];
                if sbi_count <= 20 {
                    eprintln!("[diag] SBI #{}: a7={} a0=0x{:02X} at count={}", sbi_count, a7, a0, count);
                }
                if a7 == 1 {
                    let ch = a0 as u8;
                    eprint!("{}", if ch >= 0x20 && ch < 0x7f { ch as char } else { '.' });
                    use std::io::Write;
        ...
        let result = vm.step();
        if matches!(result, StepResult::Ebreak) { break; }
    }
