...
fn main() {
    ...
    while count < max {
        ...
        if count == 178508 {
            println!("[pre-fault] count={} PC=0x{:08X} SP=0x{:08X}", count, vm.cpu.pc, vm.cpu.x[2]);

            // Compute PA for SP
            let sp = vm.cpu.x[2] as u64;
            let sp_pa = sp - 0xC0000000 as u64; // L1[772] maps VA 0xC1000000 -> PA 0x01000000
            println!("[pre-fault] SP VA=0x{:08X} -> PA=0x{:08X}", sp as u32, sp_pa as u32);
            ...
            for offset in 0..128 {
                ...
                if let Ok(word) = vm.bus.read_word(pa) {
                    if word != 0 {
                        println!("  [SP+{}] VA=0x{:08X} PA=0x{:08X}: 0x{:08X}", offset, va as u32, pa as u32, word);
                    }
                }
            }
        }
    ...
    println!("\n[post-fault] count={} PC=0x{:08X} SP=0x{:08X}", count, vm.cpu.pc, vm.cpu.x[2]);
    println!("[post-fault] RA=0x{:08X} scause=0x{:08X}", vm.cpu.x[1], vm.cpu.csr.scause);
