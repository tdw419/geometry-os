...
fn main() {
    ...
    while count < target {
        ...
        if vm.cpu.pc >= panic_start && vm.cpu.pc < panic_end { break; }
    }
    ...
    if mem_regions_ptr >= 0xC0000000 {
        ...
        for ri in 0..mem_cnt.min(8) {
            ...
            let f = vm.bus.read_word(base_pa + (ri as u64) * 12 + 8).unwrap_or(0);
            eprintln!("  memory[{}]: base=0x{:08X} size=0x{:08X} flags=0x{:08X} (end=0x{:08X})",
                ri, b, s, f, b.wrapping_add(s));
        }
    }
    ...
    if res_regions_ptr >= 0xC0000000 {
        ...
        for ri in 0..res_cnt.min(30) {
            ...
            let s = vm.bus.read_word(base_pa + (ri as u64) * 12 + 4).unwrap_or(0);
            let f = vm.bus.read_word(base_pa + (ri as u64) * 12 + 8).unwrap_or(0);
            eprintln!("  reserved[{}]: base=0x{:08X} size=0x{:08X} flags=0x{:08X} (end=0x{:08X})",
                ri, b, s, f, b.wrapping_add(s));
        }
    }
    ...
    eprintln!("Kernel L1[768..1023]: {} valid entries, last valid L1[{}]",
        valid_count, last_valid);
    eprintln!("Linear mapping covers: {}MB ({} entries * 4MB)",
        valid_count * 4, valid_count);
}
