...
fn main() {
    ...
    eprintln!("DTB magic at PA 0x{:08X}: 0x{:08X} (expect 0xD00DFEED)", dtb_pa, magic_pa);
        ...
        if count % 1000 == 0 && satp != 0 && satp & 0x80000000 != 0 {
            ...
            match result {
                mmu::TranslateResult::Ok(pa) => {
                    ...
                    eprintln!("[{}] DTB VA 0x{:08X} -> PA 0x{:08X}, word=0x{:08X} (expect 0xD00DFEED)",
                ...
                _ => {
                    eprintln!("[{}] DTB VA 0x{:08X} -> FAULT (satp=0x{:08X})", count, dtb_va, satp);
                }
            }
        }
    }
