...
fn main() {
    ...
    while count < max {
        ...
        if count >= 175_000 && count <= 180_000 {
            ...
            if count % 1000 == 0 || (satp != last_satp) {
                eprintln!("[trace {}] PC=0x{:08X} satp=0x{:08X}", count, vm.cpu.pc, satp);
            }
        }
    ...
    for i in [768, 769, 770, 771, 772, 773, 774, 775, 0, 1, 2, 3, 4, 5] {
        ...
        if val != 0 {
            eprintln!("  L1[{}] = 0x{:08X} (VA 0x{:08X}-0x{:08X})", i, val, i * 0x200000, (i+1) * 0x200000 - 1);
        }
    }
    ...
    eprintln!("\n[state] PC=0x{:08X} priv={:?} satp=0x{:08X}", vm.cpu.pc, vm.cpu.privilege, vm.cpu.csr.satp);
}
