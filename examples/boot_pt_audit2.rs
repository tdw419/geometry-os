...
fn main() {
    ...
    eprintln!("[audit] Current SATP = 0x{:08X}, pg_dir at PA 0x{:08X}", satp, pg_dir_phys);
    ...
    eprintln!("[audit] L1[773] = 0x{:08X} V={} non_leaf={}", l1_entry, is_valid, is_non_leaf);
    ...
    if is_non_leaf {
        ...
        eprintln!("[audit] L2[0x{:03X}] (VA 0xC1579000) = 0x{:08X} V={}", vpn0, l2_entry, l2_entry & 1);
        ...
        for offset in -2..=2i32 {
            ...
            eprintln!("[audit] L2[0x{:03X}] = 0x{:08X} V={}", idx, entry, entry & 1);
        }
    } else if is_valid {
    ...
    if !satp_changes.is_empty() {
        ...
        eprintln!("\n[audit] Boot page table L1[773] = 0x{:08X} (SATP was 0x{:08X})", boot_l1, boot_satp);
    }
