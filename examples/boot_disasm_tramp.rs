...
fn main() {
    ...
    for base in [0x00001000u64, 0x00001040, 0x00001080, 0x000010C0] {
        ...
        for offset in (0..64).step_by(2) {
            ...
            let word = if is_compressed { half as u32 } else { vm.bus.read_word(addr).unwrap_or(0) };
            let op = decode::decode(word);
            let op_str = format!("{:?}", op);
            let label = if addr == 0x00001048 || addr == 0x00001060 || addr == 0x000010B2 || addr == 0x000010B8 {
                " <-- STUCK"
            } else {
                ""
            };
