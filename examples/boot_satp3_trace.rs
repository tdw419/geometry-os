...
fn main() {
    ...
    if sw_a5_12 == 0xC4DC && sw_a6_24 == 0x0104AC23 && sw_a1_20 == 0xC8CC {
        ...
        vm.bus.write_word(setup_vm_va_kernel_pa_store, 0x00000013).ok(); // NOP
    ...
    for count in 1..=max_instr {
        ...
        if cur_satp != last_satp {
            satp_count += 1;
            eprintln!("[{}] SATP #{}/3: 0x{:08X} -> 0x{:08X}", count, satp_count, last_satp, cur_satp);
            last_satp = cur_satp;
        ...
        if trace_active && trace_count < max_trace {
            ...
            if opcode == 0x73 {
                ...
                if funct3 == 0 {
                    eprintln!("  [{:5}] PC=0x{:08X} ECALL (imm={})", trace_count, pc, (inst >> 20) & 0xFFF);
                }
            }
            // Check for JAL/JALR (function calls)
            else if opcode == 0x6F {
                let rd = (inst >> 7) & 0x1F;
                let imm = (((inst >> 31) & 1) << 20) | (((inst >> 12) & 0xFF) << 12) |
                    (((inst >> 20) & 1) << 11) | (((inst >> 21) & 0x3FF) << 1);
                let target = (pc as i32).wrapping_add(imm as i32) as u32;
                eprintln!("  [{:5}] PC=0x{:08X} JAL x{} -> 0x{:08X}", trace_count, pc, rd, target);
            }
            else if opcode == 0x67 {
                let rd = (inst >> 7) & 0x1F;
                ...
                if rd != 0 || rs1 != 0 { // Skip C.JR (rd=0, rs1!=0) for now
                    eprintln!("  [{:5}] PC=0x{:08X} JALR x{} x{} -> 0x{:08X}", trace_count, pc, rd, rs1, target);
                }
            }
