...
fn main() {
    ...
    eprintln!("[check] Stopped at count={}, PC=0x{:08X}", count, vm.cpu.pc);

    // Dump the full L1 page table for the current SATP
    let satp = vm.cpu.csr.satp;
    let pg_dir_phys = ((satp & 0x003F_FFFF) as u64) << 12;
    eprintln!("[pt] Root page directory at PA 0x{:08X}", pg_dir_phys);

    eprintln!("[pt] === L1 Page Table (1024 entries) ===");
    ...
    eprintln!("[pt] Total valid L1 entries: {}", valid_count);

    // For each valid non-leaf L1, dump a few L2 entries
    eprintln!("[pt] === L2 samples ===");
    for i in 0..1024 {
        let l1_addr = pg_dir_phys + (i as u64) * 4;
        let l1_pte = vm.bus.read_word(l1_addr).unwrap_or(0);
        let v = (l1_pte >> 0) & 1;
        let rwx = ((l1_pte >> 1) & 1) | ((l1_pte >> 2) & 1) | ((l1_pte >> 3) & 1);
        if v == 1 && rwx == 0 {
            // Non-leaf: dump L2
            let l2_ppn = (l1_pte >> 10) & 0xFFFFF;
            let l2_base = (l2_ppn as u64) << 12;
            eprintln!("[pt] L2 table for L1[{}]: PA 0x{:08X}", i, l2_base);
            let mut l2_valid = 0;
            for j in 0..1024 {
            ...
            eprintln!("[pt]   Total valid L2 entries: {}", l2_valid);
            if l2_valid > 10 {
                eprintln!("[pt]   (showing first 10 of {})", l2_valid);
            }
        }
    }
}
