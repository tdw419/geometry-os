...
fn main() {
    ...
    eprintln!("DTB VA (expected): 0x{:08X}", dtb_addr.wrapping_add(0xC0000000));

    // Check DTB magic at physical address
    let magic = vm.bus.read_word(dtb_addr).unwrap_or(0);
    eprintln!("DTB magic at PA 0x{:08X}: 0x{:08X} (expect 0xD00DFEED)", dtb_addr, magic);
    ...
    eprintln!("_dtb_early_va: 0x{:08X} (expect 0x{:08X})", dtb_early_va, dtb_addr.wrapping_add(0xC0000000));
    eprintln!("_dtb_early_pa: 0x{:08X} (expect 0x{:08X})", dtb_early_pa, dtb_addr);
    ...
    eprintln!("memblock memory: cnt={} max={} regions_ptr=0x{:08X}", mem_cnt, mem_max, mem_regions);
    eprintln!("memblock reserved: cnt={} regions_ptr=0x{:08X}", res_cnt, res_regions);
    ...
    if mem_cnt > 0 && mem_regions >= 0xC0000000 {
        ...
        for i in 0..mem_cnt.min(4) {
            ...
            let size = vm.bus.read_word(regions_pa + (i * 8 + 4) as u64).unwrap_or(0);
            eprintln!("  memory[{}]: base=0x{:08X} size=0x{:08X} ({}MB)", i, base, size, size / (1024*1024));
        }
    } else {
        eprintln!("  No memory regions (cnt={} regions_ptr=0x{:08X})", mem_cnt, mem_regions);
    }
    ...
    if res_cnt > 0 && res_regions >= 0xC0000000 {
        ...
        for i in 0..res_cnt.min(4) {
            let base = vm.bus.read_word(regions_pa + (i * 8) as u64).unwrap_or(0);
            let size = vm.bus.read_word(regions_pa + (i * 8 + 4) as u64).unwrap_or(0);
            eprintln!("  reserved[{}]: base=0x{:08X} size=0x{:08X} ({}KB)", i, base, size, size / 1024);
        }
    }
    ...
    eprintln!("memblock memory: cnt={} regions_ptr=0x{:08X}", mem_cnt2, mem_regions2);
    ...
    // Check SBI calls
    eprintln!("SBI console output: {} bytes", vm.bus.sbi.console_output.len());
