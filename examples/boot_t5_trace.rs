fn main() {
    ...
    while count < max_instr {
        ...
        if cur_in_fn && !in_fn {
            eprintln!("[FN_ENTRY] count={} PC=0x{:08X} from=0x{:08X}",
                count, cur_pc, last_pc);
            eprintln!("  a0=0x{:08X} a1=0x{:08X} a2=0x{:08X} ra=0x{:08X}",
                vm.cpu.x[10], vm.cpu.x[11], vm.cpu.x[12], vm.cpu.x[1]);
            eprintln!("  t3=0x{:08X} t4=0x{:08X} t5=0x{:08X} t6=0x{:08X}",
            ...
            for _ in 0..500 {
                ...
                if vm.cpu.x[30] != t5_before {
                    eprintln!("  t5 changed at PC=0x{:08X}: 0x{:08X} -> 0x{:08X}",
                        pc_before, t5_before, vm.cpu.x[30]);
                }
                
                // Stop at loop entry or return
                if vm.cpu.pc == 0xC08E5D6A {
                    eprintln!("  [loop entry] t5=0x{:08X} t4=0x{:08X} a4=0x{:08X}",
