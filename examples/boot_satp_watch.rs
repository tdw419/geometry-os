fn main() {
    ...
    let (mut vm, boot_result) = RiscvVm::boot_linux(&kernel_image, initramfs.as_deref(), 256, 17_500_000, bootargs).unwrap();

    println!("Boot result: {} instructions, entry=0x{:08X}, dtb=0x{:08X}",
        boot_result.instructions, boot_result.entry, boot_result.dtb_addr);
    println!("PC=0x{:08X} priv={:?} satp=0x{:08X}", vm.cpu.pc, vm.cpu.privilege, vm.cpu.csr.read(csr::SATP));
    println!("stvec=0x{:08X} scause=0x{:08X}", vm.cpu.csr.stvec, vm.cpu.csr.scause);

    // Check SATP write history from mmu_log
    let satp_changes: Vec<_> = vm.bus.mmu_log.iter().filter_map(|e| {
    ...
    let faults: Vec<_> = vm.bus.mmu_log.iter().filter_map(|e| {
    ...
    println!("  V={} R={} W={} X={} U={} A={} D={}",
        l1_pte & 1, (l1_pte>>1)&1, (l1_pte>>2)&1, (l1_pte>>3)&1,
        (l1_pte>>4)&1, (l1_pte>>6)&1, (l1_pte>>7)&1);
    ...
    let (mut vm2, _) = RiscvVm::boot_linux(&kernel_image, initramfs.as_deref(), 256, 17_000_000, bootargs).unwrap();
    ...
    println!("  V={} R={} W={} X={} U={} A={} D={}",
        l1_pte2 & 1, (l1_pte2>>1)&1, (l1_pte2>>2)&1, (l1_pte2>>3)&1,
        (l1_pte2>>4)&1, (l1_pte2>>6)&1, (l1_pte2>>7)&1);

    // Try even earlier: 16M
    println!("\n=== Re-booting with 16M instruction limit ===");
    let (mut vm3, _) = RiscvVm::boot_linux(&kernel_image, initramfs.as_deref(), 256, 16_000_000, bootargs).unwrap();
    ...
    println!("  V={} R={} W={} X={} U={} A={} D={}",
        l1_pte3 & 1, (l1_pte3>>1)&1, (l1_pte3>>2)&1, (l1_pte3>>3)&1,
        (l1_pte3>>4)&1, (l1_pte3>>6)&1, (l1_pte3>>7)&1);

    // Check if SATP changed between 16M and 17M
    let satp_changes_16m: Vec<_> = vm3.bus.mmu_log.iter().filter_map(|e| {
        if let mmu::MmuEvent::SatpWrite { old, new } = e { Some((*old, *new)) } else { None }
    }).collect();
    println!("SATP changes at 16M: {}", satp_changes_16m.len());

    let satp_changes_17m: Vec<_> = vm2.bus.mmu_log.iter().filter_map(|e| {
        if let mmu::MmuEvent::SatpWrite { old, new } = e { Some((*old, *new)) } else { None }
    }).collect();
