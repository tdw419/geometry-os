...
fn test_debug_monitor_assembles() {
    ...
    assert!(result.is_ok(), "debug_monitor should assemble: {:?}", result.err());
...
fn test_debug_monitor_sets_magic() {
    ...
    assert_eq!(magic, 0xDB9900, "DEBUG_MAGIC should be 0xDB9900, got 0x{:X}", magic);
...
fn test_debug_monitor_ping_response() {
    ...
    let status = vm.ram[0x0C04];
    assert_eq!(status, 2, "DEBUG_STATUS should be 2 (response ready), got {}", status);
    
    // Check response has the PING acknowledgment
    let response = vm.ram[0x0C05];
    assert_eq!(response, 0x504E4720, "Response should be PING magic, got 0x{:X}", response);
}
...
fn test_debug_monitor_regs_response() {
    ...
    println!("Child regs: r0=0x{:08X} r1=0x{:08X} r2=0x{:08X}", r0_val, r1_val, r2_val);
    ...
    assert!(r1_val != 0 || r2_val != 0, "Some register values should be non-zero from program execution");
}
