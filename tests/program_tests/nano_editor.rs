...
fn load_nano(frames: usize) -> Vm {
    let source = std::fs::read_to_string("programs/nano_editor.asm")
        .expect("nano_editor.asm not found");
...
fn test_nano_editor_assembles() {
    ...
    assert!(result.is_ok(), "nano_editor.asm should assemble: {:?}", result.err());
...
fn test_nano_editor_runs_and_shows_ui() {
    ...
    assert!(line_count >= 1, "should have at least 1 line, got {}", line_count);
    ...
    assert!(non_black > 100, "screen should have visible UI elements, got {} non-black pixels", non_black);
}
...
fn test_nano_editor_insert_chars() {
    ...
    assert_eq!(vm.ram[FB + 1], 105, "buffer should contain 'i' after typing");
