// Interactive Swarm HUD Shader - Phase 8 Beta
// 
// Visual feedback for latched wells:
//   - Selected wells glow brighter (white center)
//   - Pulsing effect based on time
//   - 2x effective strength for attraction
//
// "The Living Window" - see which well you're controlling
...
struct Well {
    pos_x: f32, pos_y: f32,
    strength: f32,
    selected: f32,  // 1.0 if latched, 0.0 otherwise
}
...
fn main(@builtin(global_invocation_id) global_id: vec3u) {
    ...
    if (y < 80u) {
        ...
        if (y >= 30u && y < 40u) {
            ...
            if (well_block < ui.well_count) {
                // Highlight selected wells in the HUD
                if (ui.wells[well_block].selected > 0.5) {
                    color = vec4f(1.0, 1.0, 1.0, 1.0);  // White for selected
                } else {
                    color = vec4f(0.3, 0.6, 1.0, 1.0);  // Blue for unselected
                }
            }
        }
        ...
        if (y >= 50u && y < 65u) {
            let text_x = x;
            // "CLICK TO DRAG" simplified indicator
            if (text_x >= 20u && text_x < 140u) {
    ...
    for (var i: u32 = 0u; i < ui.well_count; i = i + 1u) {
        ...
        // Effective strength is 2x for selected wells
        let effective_strength = well.strength * (1.0 + well.selected);
        let radius = sqrt(effective_strength) * 0.5;
        
        // Pulsing effect for selected wells
        let pulse = 1.0 + 0.2 * sin(config.time * 4.0) * well.selected;
        
        // Outer glow - brighter for selected
        if (dist < radius * 1.5 * pulse && dist > radius * pulse) {
            let alpha = 0.15 * (1.0 - (dist - radius * pulse) / (radius * 0.5 * pulse));
            let base_r = 0.3 + 0.7 * well.selected;  // White-ish for selected
            let base_g = 0.3 + 0.7 * well.selected;
            let base_b = 0.4 + 0.6 * well.selected;
            color = vec4f(base_r + alpha, base_g + alpha, base_b + alpha, 1.0);
        }
        
        // Inner circle - brighter for selected
        if (dist < radius * 0.5 * pulse) {
            let intensity = 0.5 + 0.5 * well.selected;
            color = vec4f(intensity, intensity, 0.6 + 0.4 * well.selected, 1.0);
        }
        
        // Center point - white for selected, cyan for unselected
        if (dist < 8.0) {
            if (well.selected > 0.5) {
                // Selected: bright white center
                color = vec4f(1.0, 1.0, 1.0, 1.0);
            } else {
                // Unselected: cyan center
                color = vec4f(0.5, 0.8, 1.0, 0.8);
            }
        }
        
        // Hit zone indicator (30px radius) - subtle ring for unselected
        if (well.selected < 0.5 && dist > 28.0 && dist < 32.0) {
            color = vec4f(0.3, 0.4, 0.5, 0.5);
        }
    }
